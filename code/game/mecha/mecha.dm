...
/obj/mecha
	...
	var/lights = 0
	var/lights_power = 6
	var/mob/living/silicon/ai/remote_controlled = null
...
//	var/datum/global_iterator/pr_location_temp_check //processes location temperature damage
	...
	var/max_equip = 3
	var/obj/item/weapon/tank/plasma/ptank = null //The plasma tank of this mecha. Not all mechas have this. The warthog uses it for its flamethrower.
...
/obj/mecha/proc/click_action(atom/target,mob/user)
	if(src.remote_controlled)
		if(src.remote_controlled != user ) return
		if(user.stat) return
...
/obj/mecha/verb/connect_to_port()
	...
	set category = "Exosuit Interface"
	set src in view(0)
	if(!src.occupant && !src.remote_controlled) return
...
/obj/mecha/verb/move_inside()
	...
	if(src.occupant || src.remote_controlled)
		usr << "\blue <B>The [src.name] is already being used!</B>"
...
*/
	...
	if(do_after(20))
		if(!src.occupant && !src.remote_controlled)
			moved_inside(usr)
		else if(src.occupant!=usr && src.remote_controlled != usr)
			usr << "[src.occupant ? src.occupant : src.remote_controlled] was faster. Try better next time, loser."
	return

///obj/mecha/proc/moved_inside(var/mob/living/carbon/human/H as mob)
/obj/mecha/proc/moved_inside(var/mob/living/H as mob)

	if(istype(H, /mob/living/silicon/ai)) //Do the AI's control assumption.
		/*if(H)
			H.client.eye = src
			H.client.perspective = EYE_PERSPECTIVE
			src.remote_controlled = H
			H.controlled_mecha = src
			H.cached_loc = H.loc
			H.loc = src
			src.Entered(H) //Todo: Take a look at this.
			src.Move(src.loc)
			src.log_append_to_last("[H] moved in as pilot.")
			src.icon_state = initial(icon_state)
			playsound(src, 'windowdoor.ogg', 50, 1) ///Todo: Replace with a cool sound effect.
			if(!internal_damage)
				src.remote_controlled << sound('nominal.ogg',volume=50)
			return 1

		else
			return 0*/
		//H << "Redirecting to ai_moved_inside()"
		src.ai_moved_inside(H)
	else
		//H << "Not AI."
...
/obj/mecha/proc/ai_moved_inside(var/mob/living/silicon/ai/H as mob) //TODO: Placeholder
	/*if(H /*&& H.client*/ && H in range(1))
		H.client.eye = src
		H.client.perspective = EYE_PERSPECTIVE
		H.pulling = null
...
/obj/mecha/proc/mmi_move_inside(var/obj/item/device/mmi/mmi_as_oc as obj,mob/user as mob)
	if(!mmi_as_oc.brainmob || !mmi_as_oc.brainmob.client)
		...
		return 0
	else if(occupant || remote_controlled)
		return 0
...
/obj/mecha/verb/view_stats()
	...
	usr << browse(src.get_stats_html(), "window=exosuit")
...
/obj/mecha/verb/eject()
	...
	if(usr!=src.occupant && usr != src.remote_controlled)
		return
	src.go_out()
	if(!istype(usr, /mob/living/silicon/ai)) //If not the ai.
		add_fingerprint(usr) //Add the fingerprint.
	return
...
/obj/mecha/proc/go_out()
	if(!src.occupant && !src.remote_controlled) return
	if(src.remote_controlled)
		src.remote_controlled << "Releasing control of mecha."
		//var/atom/movable/mob_container
		//mob_container = src.remote_controlled
		//src.log_message("[mob_container] released control of the mecha.")
		src.log_message("[src.remote_controlled] released control of the mecha.")
		//mob_container.loc = remote_controlled.cached_loc
		//mob_container.loc = remote_controlled.loc
		//src.Exited(mob_container)
		src.Exited(src.remote_controlled)
		src.remote_controlled.loc = src.remote_controlled.cached_loc //Move to the cached location.
		if(src.remote_controlled.client)
			src.remote_controlled.client.eye = src.remote_controlled.client.mob
			src.remote_controlled.client.perspective = MOB_PERSPECTIVE
		src.remote_controlled.controlled_mecha = null //AI no longer controlling a mecha.
		src.remote_controlled = null //No longer being remotely controlled.
		src.icon_state = initial(icon_state)+"-open"
		if(src.occupant)
			src.occupant = null //Just in case.
			world << "AI controlled mecha has occupant. Something done borked."
		return 1
		...
		/*else if(istype(occupant, /mob/living/silicon/ai))
			world << "/obj/mecha/proc/go_out(), ai subroutine. This should not run."
			//mob_container = src.remote_controlled
			/mob/living/silicon/ai/_occupant = occupant
			src.log_message("[mob_container] released control of the mecha.")
			_occupant.controlled_mecha = null //No more controlled mecha for the AI.
			//mob_container.loc = occupant.cached_loc
			//mob_container.loc = remote_controlled.loc
			_occupant.loc = occupant.cached_loc //Make the AI jump back to its original location.
			src.Exited(mob_container)
			if(src.remote_controlled.client)
				src.remote_controlled.client.eye = src.remote_controlled.client.mob
				src.remote_controlled.client.perspective = MOB_PERSPECTIVE
			src.remote_controlled = null
			src.icon_state = initial(icon_state)+"-open"
			//src.dir = dir_in
			return*/
		else if(istype(occupant, /mob/living/silicon))
			mob_container = src.occupant
		else
			return
...
/obj/mecha/proc/occupant_message(message as text)
	if(message)
		...
		if(src.remote_controlled/* && src.remote_controlled.client*/)
			src.remote_controlled << "[message]"
	return
...
/obj/mecha/Topic(href, href_list)
	...
	if(src.remote_controlled)
		if(href_list["update_content"])
			send_byjax(src.remote_controlled,"exosuit.browser","content",src.get_stats_part())
			return
		if (href_list["close"])
			return
		...
		if (href_list["view_log"])
			if(src.remote_controlled)
				src.remote_controlled << browse(src.get_log_html(), "window=exosuit_log")
				onclose(remote_controlled, "exosuit_log")
			else
				src.remote_controlled << browse(src.get_log_html(), "window=exosuit_log")
				onclose(remote_controlled, "exosuit_log")
			return
		if (href_list["change_name"])
			var/newname = strip_html_simple(input(remote_controlled,"Choose new exosuit name","Rename exosuit",initial(name)))
			if(newname)
				name = newname
			else
				if(!newname || newname == "" || newname == " ")
					alert(remote_controlled, "nope.avi")
			return
		...
		if(href_list["select_equip"])
			...
			if(equip)
				...
				send_byjax(src.remote_controlled,"exosuit.browser","eq_list",src.get_equipment_list())
...
/obj/mecha/attack_ai(var/mob/living/silicon/ai/user as mob)
	...
	//user << "/obj/mecha/attack_ai() proc called"

	if(!istype(user, /mob/living/silicon/ai))
		return
	//var/output = {"<b>Assume direct control over [src]?</b>
	//					<a href='?src=\ref[src];ai_take_control=\ref[user];'>Yes</a><br>
	//					"}
	//user << browse(output, "window=mecha_attack_ai")

	//src.ai_moved_inside(user) //Now move the ai inside the mecha.
	src.move_inside()
