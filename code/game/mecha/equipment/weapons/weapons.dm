...
/obj/item/mecha_parts/mecha_equipment/weapon/gatling
	equip_cooldown = 1
	name = "Gatling Laser"
	icon_state = "mecha_laser"
	energy_drain = 30
...
/obj/item/mecha_parts/mecha_equipment/weapon/heatray
	name = "Heat ray"
	icon_state = "mecha_exting"
	equip_cooldown = 10
	construction_time = 300
	construction_cost = list("metal"=90000)
	energy_drain = 500
	...
	action(target)
		...
		if (targloc == curloc)
			return
		set_ready_state(0)
		playsound(chassis, 'extinguish.ogg', 50, 1) //TODO: Make a cool sound effect for this.
		target_turf.hotspot_expose((PLASMA_UPPER_TEMPERATURE),1)
		chassis.use_power(energy_drain)
		chassis.log_message("Fired from [src.name], targeting [target].")
		do_after_cooldown()
		return
...
/obj/item/mecha_parts/mecha_equipment/weapon/flamethrower
	...
	action(target)  //TODO: Make this spit fire from the mecha to the target.
		if(!action_checks(target)) return
		var/turf/curloc = chassis.loc
		var/atom/targloc = get_turf(target)
		var/turf/target_turf = get_turf(target)
		...
		if (targloc == curloc)
			return
		set_ready_state(0)
		playsound(chassis, 'extinguish.ogg', 50, 1)
		//var/datum/gas_mixture/air_transfer
		//air_transfer.toxins = 1000
		//air_transfer.oxygen = 1000
		//air_transfer.temperature = 1000
		//target_turf.assume_air(air_transfer)
		target_turf.toxins = 4000
		target_turf.oxygen = 1000
		target_turf.hotspot_expose((PLASMA_UPPER_TEMPERATURE),1)
		chassis.use_power(energy_drain)
		chassis.log_message("Fired from [src.name], targeting [target].")
		do_after_cooldown()
