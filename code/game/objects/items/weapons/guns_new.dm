...
/obj/item/weapon/gun
	...
	throw_range = 10
	force = 10
	var/on = 0 //Is the gun currently activated on? That is, should it be firing?
	var/rate_of_fire = 100 //Currently only used by automatic guns.
	//Both of these are used only by automatic guns at the moment.
	//var/atom/current_loc
	//var/atom/target_loc
	var/atom/the_target
	//var/_curloc //The current location.
	//var/_targloc //The target location.
	var/mob/living/_user //The user.
	...
	projectile
		...
		gunthroat
			name = "gunthroat"
			desc = "A pirate's plaything."
			icon_state = "shotgun" //TODO: Custom sprite.
			origin_tech = "combat=5;materials=3;illegal=5"
			caliber = "shotgun"
			max_shells = 20
			force = 14.0
		...
		automatic //Hopefully someone will find a way to make these fire in bursts or something. --Superxpdude
			...
			//needs_held_update = 1 //It needs to be updated when held.
			automatic_fire = 1

			New()
				for(var/i = 1, i <= max_shells, i++)
					loaded += new /obj/item/ammo_casing/c9mm(src)
				processing_objects += src //Add this weapon to the processing objects list so that automatic firing works.
				update_icon()
			...
			process()
				...
				if(on)
					...
					if(!_user || !the_target) //If no user or target, abort.
						...
						return
					var/turf/current_loc = get_turf(_user.loc) //Update this.
					var/turf/target_loc = get_turf(the_target) //Update this.
					...
					if(target_loc.loc == current_loc.loc)
						...
						in_chamber.original = target_loc.loc
						in_chamber.loc = get_turf(_user)
						//user.next_move = world.time + 4
						in_chamber.silenced = silenced
						in_chamber.current = current_loc.loc
						in_chamber.yo = target_loc.loc.y - current_loc.loc.y
						in_chamber.xo = target_loc.loc.x - current_loc.loc.x
						spawn()
							in_chamber.process()
						sleep(1)
						in_chamber = null
	...
	afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, flag)
		...
		if(automatic_fire) //If it has automatic firing, that means it fires a round each tick.
			...
			world << "current.x = [curloc.loc.x] and current.y = [curloc.loc.y]"
			on = !on //If it's on, turn it off. If it's off, turn it on.
			//current_loc = user.loc
			the_target = target
